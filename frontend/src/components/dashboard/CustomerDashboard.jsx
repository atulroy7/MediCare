            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-border pb-5">
                <div>
                    <h2 className="text-lg font-bold font-serif text-text flex items-center gap-2">
                        ...
                        Prescription Audit & Pharmacist Notes
                    </h2>
                ...
                <Link
                    to="/prescription"
                    className="px-4 py-2.5 rounded-xl bg-primary text-white font-bold text-xs hover:bg-primary/90 transition-all flex items-center gap-2 shadow-sm"
                >
            ...
            {userPrescriptions.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {userPrescriptions.map((rx) => (
                        <div
                            key={rx.id}
                            className="bg-bg border border-border rounded-2xl p-5 space-y-3 shadow-sm hover:border-primary/30 transition-all"
                        >
                            ...
                            {rx.agentSuggestion && (
                                <div className="bg-surface p-3 rounded-xl border border-secondary/20 space-y-1">
    ...
    const OrdersSection = () => (
        <div className="bg-surface border border-border rounded-2xl p-6 shadow-sm space-y-6">
            <div className="flex items-center justify-between border-b border-border pb-5">
                <div>
                    <h2 className="text-lg font-bold font-serif text-text flex items-center gap-2">
                        <Icon name="Package" className="w-5 h-5 text-emerald-600" />
            ...
            {userOrders.length === 0 ? (
                ...
                <div className="space-y-3">
                    {userOrders.map((order, idx) => (
                        <div
                            key={order.id || idx}
                            className="bg-bg border border-border rounded-2xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4"
                        >
                            <div>
